      ...
      for (const inputFile of inputFiles) {
        ...
        await new Promise<void>((resolve) => {
          ...
          command.on('close', (data) => {
            if (cancelled) {
              onProgress({ file, status: 'error', log: `Cancelled` });
              resolve();
              return;
            }
            if (data.code !== 0) {
          ...
          command.on('error', (err) => {
            if (!cancelled) onProgress({ file, status: 'error', log: String(err) });
            resolve();
          });
          ...
          command.spawn().then((child) => {
            ...
            if (!cancelled) onProgress({ file, status: 'error', log: 'Failed to spawn ffmpeg' });
            resolve();
          });
        });
      }
      ...
      for (const inputFile of inputFiles) {
        ...
        await new Promise<void>((resolve) => {
          ...
          command.stderr.on('data', (line: string) => {
            ...
            if (timeMatch && durationInSeconds > 0 && !cancelled) {
          ...
          command.on('close', (data) => {
            if (cancelled) {
              onProgress({ file: base, status: 'error', log: `Cancelled` });
            } else if (data.code !== 0) {
          ...
          command.on('error', (err) => {
            if (!cancelled) onProgress({ file: base, status: 'error', log: String(err) });
            resolve();
          });
          ...
          command.spawn().then((child) => {
            ...
            if (!cancelled) onProgress({ file: base, status: 'error', log: 'Failed to spawn ffmpeg' });
            resolve();
          });
        });
      }
